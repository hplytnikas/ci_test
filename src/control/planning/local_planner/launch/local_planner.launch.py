...
def launch_setup(context, *args, **kwargs):
    package = get_package_share_directory("local_planner")

    controller_mode = LaunchConfiguration("controller_mode").perform(context)

    mpc_safe_config = os.path.join(package, "config", "local_planner_config_safe.yaml")
    mpc_risky_config = os.path.join(
        package, "config", "local_planner_config_risky.yaml"
    )
    mpc_rain_config = os.path.join(package, "config", "local_planner_config_rain.yaml")
    pp_config = os.path.join(package, "config", "local_planner_config_pp.yaml")
