...
static std::string s_backend; // NOLINT
...
private:
  ...
  _interpreter() {
    ...
    //  use reinterpret_cast instead of cast
...
template <typename T> struct select_npy_type {
  const static NPY_TYPES type = NPY_NOTYPE; // NOLINT
};                                          // Default
template <> struct select_npy_type<double> {
  const static NPY_TYPES type = NPY_DOUBLE; // NOLINT
};
template <> struct select_npy_type<float> {
  const static NPY_TYPES type = NPY_FLOAT; // NOLINT
};
template <> struct select_npy_type<bool> {
  const static NPY_TYPES type = NPY_BOOL; // NOLINT
};
template <> struct select_npy_type<int8_t> {
  const static NPY_TYPES type = NPY_INT8; // NOLINT
};
template <> struct select_npy_type<int16_t> {
  const static NPY_TYPES type = NPY_SHORT; // NOLINT
};
template <> struct select_npy_type<int32_t> {
  const static NPY_TYPES type = NPY_INT; // NOLINT
};
template <> struct select_npy_type<int64_t> {
  const static NPY_TYPES type = NPY_INT64; // NOLINT
};
template <> struct select_npy_type<uint8_t> {
  const static NPY_TYPES type = NPY_UINT8; // NOLINT
};
template <> struct select_npy_type<uint16_t> {
  const static NPY_TYPES type = NPY_USHORT; // NOLINT
};
template <> struct select_npy_type<uint32_t> {
  const static NPY_TYPES type = NPY_ULONG; // NOLINT
};
template <> struct select_npy_type<uint64_t> {
  const static NPY_TYPES type = NPY_UINT64; // NOLINT
};

// Sanity checks; comment them out or change the numpy type below if you're compiling on
// a platform where they don't apply
static_assert(sizeof(long long) == 8);          // NOLINT
template <> struct select_npy_type<long long> { // NOLINT
  const static NPY_TYPES type = NPY_INT64;      // NOLINT
};
static_assert(sizeof(unsigned long long) == 8);          // NOLINT
template <> struct select_npy_type<unsigned long long> { // NOLINT
  const static NPY_TYPES type = NPY_UINT64;              // NOLINT
};
...
template <typename Numeric> PyObject *get_array(const std::vector<Numeric> &v) {
  ...
  PyObject *varray = PyArray_SimpleNewFromData(1, &vsize, type, (void *)(v.data())); // NOLINT
  return varray;
}
...
template <typename Numeric> PyObject *get_2darray(const std::vector<::std::vector<Numeric>> &v) {
  ...
  PyArrayObject *varray = (PyArrayObject *)PyArray_SimpleNew(2, vsize, NPY_DOUBLE); // NOLINT
...
// TODO(_) - it should be possible to make this work by implementing
...
void plot_surface(const std::vector<::std::vector<Numeric>> &x, const std::vector<::std::vector<Numeric>> &y,
                  ...
                  const long fig_number = 0) { // NOLINT
  detail::_interpreter::get();
...
template <typename Numeric>
bool hist(const std::vector<Numeric> &y, long bins = 10, std::string color = "b", double alpha = 1.0, // NOLINT
...
inline void imshow(const unsigned char *ptr, const int rows, const int columns, const int colors,
  ...
  detail::imshow((void *)ptr, NPY_UINT8, rows, columns, colors, keywords, out); // NOLINT
}

inline void imshow(const float *ptr, const int rows, const int columns, const int colors,
                   const std::map<std::string, std::string> &keywords = {}, PyObject **out = nullptr) {
  detail::imshow((void *)ptr, NPY_FLOAT, rows, columns, colors, keywords, out); // NOLINT
}
...
bool scatter(const std::vector<NumericX> &x, const std::vector<NumericY> &y, const std::vector<NumericZ> &z,
             const double s = 1.0, // The marker size in points**2
             const std::map<std::string, std::string> &keywords = {}, const long fig_number = 0) { // NOLINT
  detail::_interpreter::get();
...
template <typename Numeric>
bool named_hist(std::string label, const std::vector<Numeric> &y, long bins = 10, std::string color = "b", // NOLINT
                double alpha = 1.0) {
  detail::_interpreter::get();
...
inline long figure(long number = -1) { // NOLINT
  detail::_interpreter::get();
  ...
  const long figureNumber = PyLong_AsLong(num); // NOLINT
...
inline bool fignum_exists(long number) { // NOLINT
  detail::_interpreter::get();
...
inline void figure_size(size_t w, size_t h) {
  ...
  PyTuple_SetItem(size, 0, PyFloat_FromDouble((double)w / dpi)); // NOLINT
  PyTuple_SetItem(size, 1, PyFloat_FromDouble((double)h / dpi)); // NOLINT
...
inline void subplot(long nrows, long ncols, long plot_number) { // NOLINT
  detail::_interpreter::get();
...
inline void subplot2grid(long nrows, long ncols, long rowid = 0, long colid = 0, long rowspan = 1, // NOLINT
                         long colspan = 1) {                                                       // NOLINT
  detail::_interpreter::get();
...
public:
  ...
  Plot(const std::string &name = "", const std::string &format = "") // NOLINT
