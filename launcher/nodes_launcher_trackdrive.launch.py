...
def generate_launch_description():
    ...
    controller_mode_arg = DeclareLaunchArgument(
        ...
        description="Mode to launch: mpc_safe, mpc_risky, mpc_rain, or pure_pursuit",
    )
    ...
    boundary_to_reference_pkg_share_dir = get_package_share_directory(
        "boundary_to_reference"
    )
    mpc_pkg_share_dir = get_package_share_directory("mpc_controller")
    local_planner_pkg_share_dir = get_package_share_directory("local_planner")
    global_planer_pkg_dir = get_package_share_directory("global_planner")
    trackdrive_handler_pkg_share_dir = get_package_share_directory("trackdrive_handler")
    ...
    slam_launch = os.path.join(
        slam_pkg_share_dir, "launch", "slam_frontend_trackdrive.launch.py"
    )
    ...
    # AutoX handler launch file
    trackdrive_handler_launch = os.path.join(
        trackdrive_handler_pkg_share_dir, "launch", "trackdrive_handler.launch.py"
    )

    # Local Planner launch file
    local_planner_launch = os.path.join(
        local_planner_pkg_share_dir, "launch", "local_planner.launch.py"
    )

    global_planner_launch = os.path.join(
        global_planer_pkg_dir, "launch", "global_planner.launch.py"
    )

    def generate_common_launch_description(perception_mode, controller_mode):
        # Launch description
        common_launch_description = [
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(trackdrive_handler_launch)
            ),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(local_planner_launch),
                launch_arguments={"controller_mode": controller_mode}.items(),
            ),
            IncludeLaunchDescription(
    ...
    def launch_sensor_fusion(lidar_mode, car_mode, perception_mode):
        # need to include the sensor fusion package once it's on main
        return (
            launch_camera(car_mode)
            + launch_lidar(lidar_mode, car_mode, perception_mode)
            + [
                IncludeLaunchDescription(
    ...
    def launch_mpc(controller_mode):
        mpc_mode = "safe"
        if controller_mode == ControllerMode.MPC_SAFE.value:
            mpc_mode = "safe"
        elif controller_mode == ControllerMode.MPC_RISKY.value:
            mpc_mode = "risky"
        elif controller_mode == ControllerMode.MPC_RAIN.value:
            mpc_mode = "rain"
        else:
            raise ValueError("MPC mode not valid")
        return [
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(mpc_launch),
                launch_arguments={"mpc_mode": mpc_mode}.items(),
            )
        ]
    ...
    def generate_launch_descriptions(context):
        ...
        if perception_mode == PerceptionMode.CAMERA_ONLY.value:
            perception_launch_description = (
                perception_launch_description + launch_camera(car_mode)
            )
            ...
            perception_launch_description = (
                perception_launch_description
                + launch_sensor_fusion(lidar_mode, car_mode, perception_mode)
            )
        else:
        ...
        if controller_mode == ControllerMode.PURE_PURSUIT.value:
            ...
            controller_mode == ControllerMode.MPC_SAFE.value
            or controller_mode == ControllerMode.MPC_RISKY.value
            or controller_mode == ControllerMode.MPC_RAIN.value
        ):
            controller_launch_description = launch_mpc(controller_mode)
        else:
            raise ValueError("Invalid controller mode")

        common_launch_description = generate_common_launch_description(
            perception_mode, controller_mode
        )
